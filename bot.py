...
async def start(message: types.Message, state: FSMContext):
    ...
    if result[0] == 0:
        if message.chat.id in admin:
            ...
            inline_buttons = [
                types.InlineKeyboardButton("Перейти в Spotty Shop", callback_data="about"),
            ]
            inline_keyboard = types.InlineKeyboardMarkup(row_width=1)
            inline_keyboard.add(*inline_buttons)
            await message.answer(
                f"Приветствую, @{user_name}! \nЧто бы задать вопрос саппорту, просто напиши его сюда, мы свяжемся с тобой. \n PS: у тебя должен быть @username!",
...
async def inline_button_handler(call: types.CallbackQuery):
    if call.data == 'about':
        await call.message.edit_text("@spottyshopbot - лучший шоп во всем тг") # Тут правила написать можно
...
async def proc(message: types.Message, state: FSMContext):
	if message.text == '⏪ Отмена':
		...
		id = data.get("uid")
		await state.finish()
		await bot.send_message(id, 'Вам поступил ответ от саппорта:\n\nТекст: {}'.format(message.text))
